...
module.exports = class MeshGateway {
	...
	emitToEventBus(source, path, data) {
		return this.callLocalExchangeMethod("eventBus", "emitMeshEvent", `${source}/${path}`, data);
	}

	async subscribeToEventBus(source, path, eventHandler) {
		return this.subscribeToMeshEvent("eventBus", `${source}/${path}`, eventHandler);
	}
	...
	#getExternalMeshService(meshClient, componentName, serviceType) {
		if (serviceType === ServiceTypes.EVENT) return meshClient.event[componentName];
	...
	async tryCallRemoteExchange(host, port, username, password, mesh, component, method, ...args) {
		let client;
		try {
			client = this.#clientFactory.createMeshClient(username != null, host, port);
